...
def main(args):
    ...
    for ii in range(len_imgs):
        ...
        # print(inputName)
        gt_img = '/'.join(inputName.split('/')[:-2])+'/gt/'+'/'.join(inputName.split('/')[-1:])[:-4]+'.png'
        # print(gt_img)
        ...
        for x in mask_test:
            ...
            for y in x:
                ...
                tmpp.append(tmp)
            mask_.append(tmpp)
        print(np.array(mask_).shape)
        mask_ = np.array(mask_)
        cv2.imwrite(
            f"{str(visual_dir)}/{base_name.split('.')[0]}_{visual_type}.png",
            np.concatenate([vis_img, _gt_img, mask_], axis=1)
        )
        # print(curves_mask)
        # break
